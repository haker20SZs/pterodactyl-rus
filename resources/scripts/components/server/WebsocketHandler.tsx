...
export default () => {
    ...
    const connect = (uuid: string) => {
        ...
        socket.on('daemon error', (message) => {
            console.warn('Получил сообщение об ошибке от сокета демона:', message);
        });
        ...
        socket.on('jwt error', (error: string) => {
            setConnectionState(false);
            console.warn('Ошибка проверки JWT из крыльев:', error);

            if (reconnectErrors.find((v) => error.toLowerCase().indexOf(v) >= 0)) {
                updateToken(uuid, socket);
            } else {
                setError(
                    'Произошла ошибка при проверке учетных данных, предоставленных для веб-сокета. Пожалуйста, обновите страницу.'
                );
            }
        });
