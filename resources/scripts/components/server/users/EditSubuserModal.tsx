    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, 'Адреса электронной почты не должны превышать 191 символ.')
            ...
            <Form>
                <div css={tw`flex justify-between`}>
                    <h2 css={tw`text-2xl`} ref={ref}>
                        {subuser
                            ? `${canEditUser ? 'Изменить' : 'Посмотреть'} разрешения на ${subuser.email}`
                            : 'Создание нового подпользователя'}
                    </h2>
                    <div>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            При создании или изменении других пользователей можно выбирать только те разрешения, которые в данный момент назначены вашей учетной записи.
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            ...
                            description={
                                'Введите адрес электронной почты пользователя, которого вы хотите пригласить в качестве субпользователя для этого сервера.'
                            }
                        />
                    </div>
                )}
