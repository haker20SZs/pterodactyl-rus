    ...
    return (
        <GreyRowBox css={tw`mb-2`}>
            ...
            <div css={tw`ml-4`}>
                ...
                <p css={tw`text-2xs text-zinc-600 dark:text-zinc-500 uppercase hidden md:block`}>2FA Включить</p>
            </div>
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center`}>
                    {subuser.permissions.filter((permission) => permission !== 'websocket.connect').length}
                </p>
                <p css={tw`text-2xs text-zinc-600 dark:text-zinc-500 uppercase`}>Разрешения</p>
            </div>
            {subuser.uuid !== uuid && (
                <>
