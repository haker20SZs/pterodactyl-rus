...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    const diskLimit = server.limits.disk !== 0 ? bytesToString(mbToBytes(server.limits.disk)) : 'Без лимита';
    const memoryLimit = server.limits.memory !== 0 ? bytesToString(mbToBytes(server.limits.memory)) : 'Без лимита';
    const cpuLimit = server.limits.cpu !== 0 ? server.limits.cpu + ' %' : 'Без лимита';
                ...
                {!stats || isSuspended ? (
                    ...
                    <React.Fragment>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            ...
                            <p css={tw`text-xs text-zinc-600 dark:text-zinc-400 text-center mt-1`}>из {cpuLimit}</p>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                            ...
                            <p css={tw`text-xs text-zinc-600 dark:text-zinc-400 text-center mt-1`}>из {memoryLimit}</p>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                            ...
                            <p css={tw`text-xs text-zinc-600 dark:text-zinc-400 text-center mt-1`}>из {diskLimit}</p>
                        </div>
                    </React.Fragment>
                )}
            </div>
