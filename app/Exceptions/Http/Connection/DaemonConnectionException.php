...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = 'Не удалось установить соединение с машиной, на которой запущен этот сервер. Пожалуйста, попробуйте еще раз.';
        } else {
            $message = sprintf('Произошла ошибка при взаимодействии с машиной, на которой запущен этот сервер. Эта ошибка была зарегистрирована, пожалуйста, попробуйте еще раз. (code: %s) (request_id: %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('На удаленном узле произошла ошибка: %s. (идентификатор запроса: %s)', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
